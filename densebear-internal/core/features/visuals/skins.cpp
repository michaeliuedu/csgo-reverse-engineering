...
//INLINE TO AVOID CPP LINK ERR: TODO: MOVE TO NAMESPACE
inline bool apply_glove_model(attributable_item_t* glove, const char* model) noexcept {
	player_info_t info;
...
inline bool apply_glove_skin(attributable_item_t* glove, int item_definition_index, int paint_kit, int model_index, int entity_quality, float fallback_wear) noexcept {
...
inline bool apply_skin(attributable_item_t* skin, const char* model, int item_definition_index, int paint_kit, int model_index, int entity_quality, float fb_wear) {
...
inline void run() {
	...
	if (!local_player)
		...
		if (glove)
		{
			...
			//case 1:
				...
				//apply_glove_skin(glove, GLOVE_STUDDED_BLOODHOUND, 10006, index_blood, 3, g_wear);
			//	break;
			//case 2:
				apply_glove_model(glove, model_sport);
				apply_glove_skin(glove, GLOVE_SPORTY, 10048, index_blood, 3, g_wear);
			...
			//	break;
			//case 7:
				//apply_glove(glove, model_hydra, GLOVE_HYDRA, 1, GET_INDEX(model_hydra), 3, g_wear);
			//	break;
			//}
	...
	// skinchanger
		...
		for (size_t i = 0; my_weapons[i] != INVALID_EHANDLE_INDEX; i++) {
			...
			if (weapon->client_class()->class_id == class_ids::cknife) {
				...
				//case 1:
				//	apply_skin(weapon, model_bayonet, WEAPON_BAYONET, config_system.item.paint_kit_index_knife, GET_INDEX(model_bayonet), 3, wear);
				//	break;
				//case 2:
				//	apply_skin(weapon, model_m9, WEAPON_KNIFE_M9_BAYONET, config_system.item.paint_kit_index_knife, GET_INDEX(model_m9), 3, wear);
				//	break;
				//case 3:
					apply_skin(weapon, model_karambit, WEAPON_KNIFE_KARAMBIT, 415, GET_INDEX(model_karambit), 3, wear);
				//	break;
				//case 4:
				...
				//	break;
				//case 19:
					//apply_skin(weapon, model_css, WEAPON_KNIFE_CSS, 1, GET_INDEX(model_css), 3, wear);
					//break;
				//}
			}
	...
	/*		case WEAPON_USP_SILENCER:
			...
			case WEAPON_AK47:
				weapon->fb_paintkit() = 259, weapon->fb_wear() = wear;
				break;

			case WEAPON_AWP:
				weapon->fb_paintkit() = 344, weapon->fb_wear() = wear;
				break;
	/*		case WEAPON_SCAR20:

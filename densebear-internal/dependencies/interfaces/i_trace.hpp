...
struct ray_t {
	...
	void initialize(vec3_t const& start, vec3_t const& end, vec3_t const& mins, vec3_t const& maxs) {
		...
		m_start_offset.x = maxs.x + mins.x;
		m_start_offset.y = maxs.y + mins.y;
		m_start_offset.z = maxs.z + mins.z;

		m_start_offset *= 0.5f;

		m_start.x = start.x + m_start_offset.x;
		m_start.y = start.y + m_start_offset.y;
		m_start.z = start.z + m_start_offset.z;
